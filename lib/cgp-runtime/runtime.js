 ...
 * @param {string} opts.columnValue     Column interface string (header cell).
 * @returns {object}  Four-facet spike entry conforming to spike.schema.json.
 */
export function buildSpikeEntry({ systemId, observatronId, columnIndex, columnValue }) {
    ...
    for (const el of document.querySelectorAll('[cgp-id]')) {
      ...
      target.addEventListener('drop', async (e) => {
        ...
        for (let colIdx = 0; colIdx < numCols; colIdx++) {
          const columnValue = cells[colIdx] !== undefined ? cells[colIdx].trim() : '';
