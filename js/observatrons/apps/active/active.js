// active.js — Observatron wired to canonical state via StateClient.
// Renders spikes from the authoritative /api/state pipeline.
...
function rebuildMesh(obs) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const mBit = (facets?.['/meaning']?.key?.length > 0) ? 1 : 0;
    const sBit = (facets?.['/structure']?.key?.length > 0) ? 1 : 0;
    ...
    const meaning = facets?.['/meaning']?.key?.[0] || entry.url.split('/').pop();
...
// ── StateClient subscription (canonical pipeline) ─────────────

connectToState({
  onUpdate(state, seq) {
    const obs0 = state.observatrons?.[0];
    if (!obs0) return;
    ...
    spikeList.length = 0;
    renderedUrlSet.clear();

    for (const spike of (obs0.spikes || [])) {
      const url = spike['/context']?.anchor?.[0] || '';
      const timestamp = spike['/context']?.timestamp?.[0] || '';
      spikeList.push({ url, facets: spike, timestamp });
