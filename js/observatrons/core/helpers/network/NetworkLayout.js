...
/**
 * NetworkLayout — Layout-only — positions caller-provided observatron mesh
 * groups in a grid. Does not generate spike data.
 */
export class NetworkLayout {
   ...
   */
  constructor({ pivot, camera, colorScheme }) {
    this._pivot = pivot;
    this._camera = camera;
  ...
  /**
   * Position caller-provided observatron mesh groups in a grid layout.
   * Disposes any previously laid-out nodes first.
   * @param {{ group: THREE.Group, dirs: THREE.Vector3[], spikes: object[], regionStats: object }[]} entries
   */
  setNodes(entries) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this._nodes.push({ group: nodeGroup, box, label, dirs: entry.dirs, spikes: entry.spikes, regionStats: entry.regionStats });
    }
  }
